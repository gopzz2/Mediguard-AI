...
// const urgencyConfig = {
//   Emergency: { cls: "emergency", icon: "🚨", sub: "Seek immediate emergency care" },
...
//     try {
//       const res  = await fetch(`${BACKEND}/api/symptom-analyse`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ symptoms, language }) });
...
//           <textarea rows={3} placeholder="e.g. fever + headache + body pain + cold" value={symptoms} onChange={e => setSymptoms(e.target.value)} onKeyDown={e => e.key==="Enter"&&!e.shiftKey&&(e.preventDefault(),analyse())} />
...
//             <div>
//               <div className="urgency-text">Urgency Level: {result.urgencyLevel}</div>
...
//               <div className="confidence-label"><span>Detection Confidence</span><span style={{color:"var(--accent)",fontWeight:700}}>{result.confidenceScore}%</span></div>
//               <div className="confidence-track"><div className="confidence-fill" style={{width:`${result.confidenceScore}%`}}/></div>
//             </div>
//           </div>

//           {/* Possible Conditions */}
//           <p className="section-label">Possible Conditions (Multi-Label Analysis)</p>
//           <div className="result-grid">
//             {result.possibleConditions?.map((c,i) => (
//               <div className="result-item" key={i}>
//                 <div className="result-item-header">
//                   <span className="result-item-name">{c.name}</span>
//                   <span className={`badge ${likelihoodBadge[c.likelihood]||"badge-gray"}`}>{c.likelihood}</span>
//                 </div>
//                 <p className="result-item-desc">{c.description}</p>
//               </div>
//             ))}
//           </div>
...
//             <div className="card" style={{padding:"16px 20px"}}>
//               <ul className="info-list">{result.recommendations.map((r,i)=><li key={i}>{r}</li>)}</ul>
//             </div>
//           </>}

//           {/* Sources */}
//           {result.sources?.length > 0 && <>
//             <p className="section-label">Knowledge Sources (RAG Pipeline)</p>
...
//               <p style={{fontSize:12,color:"var(--text-muted)"}}>Find verified hospitals and clinics near your location</p>
//             </div>
//             <a className="map-btn" href="https://www.google.com/maps/search/hospitals+near+me" target="_blank" rel="noreferrer">
//               <span>📍</span> Open Google Maps — Hospitals Near Me
//             </a>
//           </div>

//           {result.disclaimer && <div className="disclaimer" style={{marginTop:12}}>⚠️ {result.disclaimer}</div>}
//         </div>
//       )}
//     </div>
//   );
